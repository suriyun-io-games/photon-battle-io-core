...
public class UIEquipButton : MonoBehaviour
{
    ...
    private bool IsEquipped()
    {
        ...
        if (uiProductData.productData is WeaponData)
        {
            for (var i = 0; i < GameInstance.AvailableWeapons.Count; ++i)
            {
                var item = GameInstance.AvailableWeapons[i];
                if (item == uiProductData.productData)
                {
                    return i == uiMainMenu.SelectWeapon;
                }
            }
        }
